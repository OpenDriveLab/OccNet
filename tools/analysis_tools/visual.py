...
def render_sample_data(
    ...
    for ind, cam in enumerate(cams):
        ...
        print('pred box num:', len(pred_data['results'][sample_toekn]))
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            # the boxes is in the global system 
            boxes = [Box(record['translation'], record['size'], Quaternion(record['rotation']),
                     ...
                     pred_data['results'][sample_toekn] if record['detection_score'] > score]
            # print(boxes[0])
            # obtain predicted data in the camera system
            data_path, boxes_pred, camera_intrinsic = get_predicted_data(sample_data_token,
                                                                         box_vis_level=box_vis_level, pred_anns=boxes)
            
            # obtain gt data in the camera system
...
if __name__ == '__main__':
    ...
    data_dir = 'val/exps/bev_tiny_det_v2_deterministic/Thu_Jan_26_00_43_33_2023/pts_bbox' 
    data_path = os.path.join(data_dir, 'results_nusc.json')
    bevformer_results = mmcv.load(data_path)
    
    scene_token_path = 'data/nuscenes_occupancy/validation_scene_tokens.json'
    scene_token_infos = mmcv.load(scene_token_path)
    
    # visual_scene = 'scene-0099'
    visual_scene = 'scene-0626'
    sample_token_list = list(scene_token_infos[visual_scene]['sampel_tokens'].values())
    
    score = 0.25
    save_dir = os.path.join(data_dir, 'visualization', visual_scene, str(score))
    os.makedirs(save_dir, exist_ok=True)

    
    for id in range(len(sample_token_list)):
        out_path = '{}/{:03d}'.format(save_dir, id)
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=out_path, score=score)
